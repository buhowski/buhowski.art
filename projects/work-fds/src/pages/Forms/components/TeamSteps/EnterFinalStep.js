...
const EnterFinalStep = ({ devForm }) => {
	...
	const content = (
		<div className='finalFormBlock'>
			...
			{devForm && <div className='formField formField--mobile'>
				<SingleDropFile
					labelClassName={`formLabel required ${
						invalidInputs.includes('step4dropFileCV') ? 'invalid' : ''
					}`}
					dropInput={`step4dropFileCV`}
					dropInputId={'step4dropFileCV'}
					htmlForLabel='step4dropFileCV'
				/>
			</div>}
		</div>
	);
